...
一個使用原生 HTML/CSS/JavaScript 打造的極簡行事曆原型。核心目標是提供漂亮的月曆視圖、可互動的日期切換，以及選定日期後的全天時段清單，方便進一步擴充成完整的代辦或行程工具。
...
- **Task List Panel（行程清單面板）**：透過 Calendar Panel 上的「Task List」按鈕彈出，顯示所有已建立的行程摘要（如 `2026/01/21 健身房`），隨新增事件即時更新。
- **Day View Panel（日檢視面板）**：顯示被選日期、星期文字、Day Form 以及 24 小時時段清單。只有在點選日期後才會出現。
- **Day Form（日檢視表單）**：於 Day View 內用選單挑開始/結束時間並輸入標題，針對目前選定日期快速建立行程。
- **行程渲染**：採 30 分鐘解析度、顯示疊加色塊；重疊時段會堆疊顯示並維持短行程在上方，同時只標示整點時間，讓畫面簡潔。
...
| 名稱 | 說明 |
| --- | --- |
| **Calendar Panel** | 左側月曆面板，包括標題、月份導航、Sun~Sat 標頭與 6x7 日期按鈕。初始畫面只顯示這個區塊。 |
...
| **Event Block** | 顯示在 Hour Blocks 內的行程色塊（`day-view__event`），會標註開始/結束時間與標題，且支援重疊堆疊。 |
...
3. 部署：把檔案推到 GitHub，啟用 GitHub Pages（Source 選 main / root），即可透過 `https://<username>.github.io/calendar/` 取用。
